...
export const LearningMilestonesWidget: React.FC<LearningMilestonesWidgetProps> = ({
  ...
  const weeklyTarget = safeNumber(activeGoal?.weekly_study_hours || stats?.weekly_committed_hours, 12);
  const completedHours = activeCourses
    .filter((course) => course.node_status === 'completed')
    .reduce((sum, course) => sum + safeNumber(course.duration_hours, 0), 0);
  const hoursLogged = completedHours > 0 ? Math.min(weeklyTarget, completedHours) : 8.5;
  const nextCourse =
    activeCourses.find((course) => course.node_status === 'in_progress') ||
    activeCourses.find((course) => course.node_status !== 'completed') ||
    null;
  const nextNodeTitle = nextCourse?.title || 'Vector Search & Embeddings Diagnostic';
  const streakDays = Math.max(3, Math.min(14, (stats?.passed_assessments_count || 2) + 3));
    ...
    <section className="w-full bg-white border border-slate-200 rounded-2xl p-6 shadow-sm dark:bg-slate-900/80 dark:border-slate-800/80 dark:shadow-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-500/30 dark:bg-emerald-950/40">
          ...
          <p className="mt-2 text-xl font-extrabold text-slate-900 dark:text-white">{streakDays} Days Active 🔥</p>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/60">
          ...
          <p className="mt-2 text-xl font-extrabold text-slate-900 dark:text-white">
            {hoursLogged.toFixed(1)} / {weeklyTarget} hrs
          </p>
